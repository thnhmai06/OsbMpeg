...
public sealed class CanvasVideoFrame(Canvas canvas) : IVideoFrame
{
    ...
    public void Serialize(Stream pipe) => pipe.Write(canvas.Rgb);
    public Task SerializeAsync(Stream pipe, CancellationToken token) => pipe.WriteAsync(canvas.Rgb, token).AsTask();
}
