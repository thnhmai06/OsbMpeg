...
/// <summary>Collapses runs of same-kind SbValueCommand (Fade/MoveX/MoveY/Scale/
...
/// are already minimally windowed by FlagWindows, not per-frame sampled.</summary>
public static class MergeAdjacentCommands
{
    ...
    private static List<SbValueCommand> MergeRun(List<SbValueCommand> sorted)
    {
        ...
        foreach (var c in sorted)
        {
            ...
            {
                var last = merged[^1];
                merged[^1] = new SbValueCommand { Kind = last.Kind, Easing = last.Easing, StartMs = last.StartMs, EndMs = c.EndMs, Start = last.Start, End = c.End };
            }
            else
            {
                merged.Add(c);
    ...
    private static bool CanMerge(SbValueCommand a, SbValueCommand b)
    {
        ...
        if (Math.Abs(a.EndMs - b.StartMs) > TimeEpsilonMs) return false;
        if (Math.Abs(a.End - b.Start) > ValueEpsilon) return false;

        var durA = a.EndMs - a.StartMs;
        var durB = b.EndMs - b.StartMs;
        if (durA <= 0 || durB <= 0)
            return Math.Abs(a.Start - a.End) < ValueEpsilon && Math.Abs(b.Start - b.End) < ValueEpsilon;

        var slopeA = (a.End - a.Start) / durA;
        var slopeB = (b.End - b.Start) / durB;
        return Math.Abs(slopeA - slopeB) < SlopeEpsilon;
    }
}
