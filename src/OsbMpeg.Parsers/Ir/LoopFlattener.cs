...
/// <summary>Expands SbLoop into plain absolute-time commands — .osb's L command replays its
...
/// object possibly be alive" bound it's used for, but not precise enough to reuse here.</summary>
public static class LoopFlattener
{
    public static List<SbCommand> Flatten(List<SbCommand> commands)
    {
        ...
        foreach (var c in commands)
        {
            ...
            var cycleLength = 0.0;
            foreach (var fc in flatChildren)
                if (fc is not SbTrigger)
                    cycleLength = Math.Max(cycleLength, fc.EndMs);
            ...
            for (var i = 0; i < iterations; i++)
            {
                ...
                foreach (var fc in flatChildren)
                    result.Add(Shift(fc, shift));
            }
        }
        return result;
    }

    private static SbCommand Shift(SbCommand c, double shift) => c switch
    {
