...
public abstract class SbObject
{
    ...
    /// <summary>
    ///     Per osu! semantics: only objects with >=1 non-trigger command are ever
    ///     instantiated by the renderer (HasCommands excludes TriggerGroups).
    /// </summary>
...
/// <summary>
///     osu! derives EVERY frame's file — including frame 0 — from BasePath via
...
///     client, producing the wrong file.
/// </summary>
public sealed class SbAnimation : SbObject
{
    ...
    public AssetId FramePath(int frameIndex)
    {
        return new AssetId(BasePath.RelativePath.Replace(".", $"{frameIndex}."));
    }
}