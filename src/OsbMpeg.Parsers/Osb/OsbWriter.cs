...
/// <summary>
///     Serializes a <see cref="SbDocument" /> to .osb text. Applies the two shorthand
...
{
    private static readonly SbLayer[] LayerOrder = 
    ...
    private static void WriteObjectHeader(TextWriter w, SbObject obj)
    {
        switch (obj)
        {
            ...
            case SbAnimation a:
                w.WriteLine(
                    $"Animation,{obj.Layer},{obj.Origin},\"{a.BasePath}\",{FormatFloat(obj.X)},{FormatFloat(obj.Y)},{a.FrameCount},{FormatFloat((float)a.FrameDelayMs)},{a.LoopType}");
                break;
            default:
    ...
    /// <summary>
    ///     Pairs VectorScaleX/VectorScaleY into a single V line (osu! has no VX/VY
    ///     acronym — vector scale is always written as one command with both axes). Every
    ///     other command kind maps to its own acronym independently (MX/MY are legal alone).
    /// </summary>
        ...
        for (var i = 0; i < commands.Count; i++)
        {
            ...
            if (commands[i] is SbValueCommand { Kind: SbCommandKind.VectorScaleX } vx)
            {
                var j = commands.FindIndex(i + 1, c => c is SbValueCommand { Kind: SbCommandKind.VectorScaleY } vy
                                                       && vy.StartMs.IsEqual(vx.StartMs) && vy.EndMs.IsEqual(vx.EndMs) &&
                                                       vy.Easing == vx.Easing);
                if (j < 0)
                    throw new InvalidOperationException(
                        "VectorScaleX with no matching VectorScaleY at the same time span — osu! has no standalone VX/VY acronym.");
                consumed.Add(j);
    ...
    private static void WriteCommand(TextWriter w, SbCommand cmd, int depth)
    {
        ...
        switch (cmd)
        {
            ...
            case SbValueCommand v:
                WriteCommandLine(w, indent, Acronym(v.Kind), v.Easing, v.StartMs, v.EndMs, [FormatFloat(v.Start)],
                    [FormatFloat(v.End)]);
                break;
            ...
            case SbColourCommand c:
                WriteCommandLine(w, indent, "C", c.Easing, c.StartMs, c.EndMs,
                    [
                        c.Start.R.ToString(CultureInfo.InvariantCulture),
                        c.Start.G.ToString(CultureInfo.InvariantCulture),
                        c.Start.B.ToString(CultureInfo.InvariantCulture)
                    ],
                    [
                        c.End.R.ToString(CultureInfo.InvariantCulture), c.End.G.ToString(CultureInfo.InvariantCulture),
                        c.End.B.ToString(CultureInfo.InvariantCulture)
                    ]);
                break;

            case SbFlagCommand f:
                WriteCommandLine(w, indent, "P", f.Easing, f.StartMs, f.EndMs, [FlagLetter(f.Kind).ToString()],
                    [FlagLetter(f.Kind).ToString()]);
                break;

            case SbLoop loop:
                w.WriteLine($"{indent}L,{FormatTime(loop.StartMs)},{loop.Count}");
                foreach (var child in NormalizeCommands(loop.Children))
                    WriteCommand(w, child, 2);
                break;

            case SbTrigger trigger:
                w.WriteLine(
                    $"{indent}T,{trigger.Name},{FormatTime(trigger.StartMs)},{FormatTime(trigger.EndMs)},{trigger.Group}");
    ...
    /// <summary>
    ///     Writes one command line with both shorthand omissions applied: the endtime
    ///     field is left blank when it equals StartTime, and endValues is dropped entirely when
    ///     it's identical (field-for-field, as formatted text) to startValues.
    /// </summary>
    private static void WriteCommandLine(TextWriter w, string indent, string acronym, SbEasing easing, double startMs,
        double endMs, string[] startValues, string[] endValues)
    {
        var startTime = FormatTime(startMs);
        var endTime = startTime == FormatTime(endMs) ? "" : FormatTime(endMs);
        var fields = new List<string>
            { acronym, ((int)easing).ToString(CultureInfo.InvariantCulture), startTime, endTime };
    ...
    private static string Acronym(SbCommandKind kind)
    {
        return kind switch
        {
    ...
    private static char FlagLetter(SbCommandKind kind)
    {
        return kind switch
        {
    ...
    {
        return ((long)Math.Round(ms, MidpointRounding.AwayFromZero)).ToString(CultureInfo.InvariantCulture);
    }
