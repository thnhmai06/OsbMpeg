...
/// <summary>
///     Handwritten recursive-descent-over-indentation parser for the .osbv source
...
{
    public static OsbvDocument ParseFile(string path)
    {
        ...
        foreach (var raw in lines)
        {
            ...
            if (!sawHeader)
            {
                if (trimmedStart.TrimEnd() != "OsbV: 1")
                    throw new OsbvParseException(lineNo,
                        $"expected version header \"OsbV: 1\", got \"{trimmedStart.TrimEnd()}\"");
                sawHeader = true;
                continue;
            }
            ...
            if (stack.Count == 0 || stack[^1].Depth != depth - 1)
                throw new OsbvParseException(lineNo,
                    $"unexpected indentation (depth {depth} with no open depth-{depth - 1} block)");
    ...
    private static OsbvObject ParseObjectHeader(List<string> f, int lineNo)
    {
        switch (f[0])
        {
            ...
            case "AnimationVideo":
                if (f.Count is < 7 or > 10)
                    throw new OsbvParseException(lineNo,
                        $"AnimationVideo,layer,origin,\"path\",x,y,startTime[,fps[,videoStartTime[,videoEndTime]]] — got {f.Count} fields");
    ...
    /// <summary>
    ///     Returns 1 command normally, 2 for M/V (expanded to X+Y components, matching
    ///     how OsbReader expands OsuParsers' Movement/VectorScale). Wrapped in a list so the
    ///     caller can special-case "did this line open an L block" without a type switch.
    /// </summary>
        ...
        switch (f[0])
        {
            case "T":
            {
                ...
                [
                    new SbTrigger
                    {
                        Name = f[1], StartMs = ParseDouble(f[2], lineNo), EndMs = ParseDouble(f[3], lineNo), Group = group,
                        Children = []
                    }
                ];
            }
        ...
        switch (f[0])
        {
            ...
            case "M":
            {
                ...
                [
                    new SbValueCommand
                    {
                        Kind = SbCommandKind.MoveX, StartMs = startMs, EndMs = endMs, Easing = easing, Start = sx,
                        End = ex
                    },
                    new SbValueCommand
                    {
                        Kind = SbCommandKind.MoveY, StartMs = startMs, EndMs = endMs, Easing = easing, Start = sy,
                        End = ey
                    }
                ];
            }
            ...
            case "V":
            {
                ...
                [
                    new SbValueCommand
                    {
                        Kind = SbCommandKind.VectorScaleX, StartMs = startMs, EndMs = endMs, Easing = easing,
                        Start = sx, End = ex
                    },
                    new SbValueCommand
                    {
                        Kind = SbCommandKind.VectorScaleY, StartMs = startMs, EndMs = endMs, Easing = easing,
                        Start = sy, End = ey
                    }
                ];
            }
            ...
            case "C":
            {
                ...
                var end = f.Count > 7
                    ? new SbColor(ParseByte(f[7], lineNo), ParseByte(f[8], lineNo), ParseByte(f[9], lineNo))
                    : start;
                return
                [
                    new SbColourCommand { StartMs = startMs, EndMs = endMs, Easing = easing, Start = start, End = end }
                ];
            }
            ...
            case "P":
                ...
                [
                    new SbFlagCommand
                        { Kind = ParseFlag(f[4], lineNo), StartMs = startMs, EndMs = endMs, Easing = easing }
                ];
    ...
    private static SbValueCommand ScalarCommand(SbCommandKind kind, List<string> f, int valueIndex, SbEasing easing,
        double startMs, double endMs, int lineNo)
    {
        ...
        return new SbValueCommand
            { Kind = kind, StartMs = startMs, EndMs = endMs, Easing = easing, Start = start, End = end };
    }

    private static SbCommandKind ParseFlag(string s, int lineNo)
    {
        return s switch
        {
    ...
    {
        return Enum.TryParse<SbLayer>(s, out var v)
            ? v
            : throw new OsbvParseException(lineNo, $"unknown layer \"{s}\"");
    }

    private static SbOrigin ParseOrigin(string s, int lineNo)
    {
        return Enum.TryParse<SbOrigin>(s, out var v)
            ? v
            : throw new OsbvParseException(lineNo, $"unknown origin \"{s}\"");
    }
    ...
    private static double ParseDouble(string s, int lineNo)
    {
        return double.TryParse(s, NumberStyles.Float, CultureInfo.InvariantCulture, out var v)
            ? v
            : throw new OsbvParseException(lineNo, $"expected number, got \"{s}\"");
    }

    private static float ParseFloat(string s, int lineNo)
    {
        return (float)ParseDouble(s, lineNo);
    }
    ...
    private static int ParseInt(string s, int lineNo)
    {
        return int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var v)
            ? v
            : throw new OsbvParseException(lineNo, $"expected integer, got \"{s}\"");
    }

    private static byte ParseByte(string s, int lineNo)
    {
        return byte.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out var v)
            ? v
            : throw new OsbvParseException(lineNo, $"expected byte 0-255, got \"{s}\"");
    }
    ...
    /// <summary>
    ///     Comma-splits a line, treating '"'-delimited spans as literal (commas inside
