...
/// <summary>
///     Bakes an AnimationVideo group's script into per-tile M/V/R/F/C/P commands — .osb
...
///     position convention), Scale/VectorScale, Rotate, Fade, Color, Additive, flip (P,H/P,V).
...
///     fire time, which doesn't exist until a real playthrough reaches it). Fade/Color/Additive
...
///     sync on the same real event. Anything else inside a Trigger is rejected.
/// </summary>
public sealed class GroupTransformBaker
{
    ...
    {
        foreach (var c in from c in trigger.Children
                 let allowed = c is SbValueCommand { Kind: SbCommandKind.Fade } or SbColourCommand
                     or SbFlagCommand { Kind: SbCommandKind.Additive }
                 where !allowed
                 select c)
        {
    ...
    /// <summary>
    ///     baseCenterX/Y is the tile's un-transformed storyboard center (auto-cover
    ///     mapping only, no script applied); baseScale is the mapping's StoryboardScale. Returns
    ///     the commands to attach to a Center-origin sprite/animation covering
    ///     [localStartMs,localEndMs] (extraction-local time) at storyboardTimeOffsetMs.
    /// </summary>
    public (float X, float Y, List<SbCommand> Commands) Bake(double baseCenterX, double baseCenterY, double baseScale,
        double localStartMs, double localEndMs, double storyboardTimeOffsetMs)
    {
        ...
        var commands =
            new List<SbCommand>(_triggers); // shared instances, never mutated — safe to reuse across every tile

        if (positionConstant)
        {
            commands.Add(new SbValueCommand
            {
                Kind = SbCommandKind.MoveX, StartMs = absStart, EndMs = absEnd, Start = (float)first.X,
                End = (float)first.X
            });
            commands.Add(new SbValueCommand
            {
                Kind = SbCommandKind.MoveY, StartMs = absStart, EndMs = absEnd, Start = (float)first.Y,
                End = (float)first.Y
            });
            commands.Add(new SbValueCommand
            {
                Kind = SbCommandKind.VectorScaleX, StartMs = absStart, EndMs = absEnd, Start = (float)first.ScaleX,
                End = (float)first.ScaleX
            });
            commands.Add(new SbValueCommand
            {
                Kind = SbCommandKind.VectorScaleY, StartMs = absStart, EndMs = absEnd, Start = (float)first.ScaleY,
                End = (float)first.ScaleY
            });
            if (!first.Rotation.IsEqual(0.0, 1e-6))
                commands.Add(new SbValueCommand
                {
                    Kind = SbCommandKind.Rotate, StartMs = absStart, EndMs = absEnd, Start = (float)first.Rotation,
                    End = (float)first.Rotation
                });
            if (first.FlipH)
        ...
        if (fadeConstant && !first.Alpha.IsEqual(1f, 1e-4f))
            commands.Add(new SbValueCommand
            {
                Kind = SbCommandKind.Fade, StartMs = absStart, EndMs = absEnd, Start = first.Alpha, End = first.Alpha
            });
        if (colourConstant && first.Colour != SbColor.White)
            commands.Add(new SbColourCommand
                { StartMs = absStart, EndMs = absEnd, Start = first.Colour, End = first.Colour });
        ...
        for (var i = 0; i < samples.Count - 1; i++)
        {
            ...
            {
                commands.Add(new SbValueCommand
                    { Kind = SbCommandKind.MoveX, StartMs = t0, EndMs = t1, Start = (float)a.X, End = (float)b.X });
                commands.Add(new SbValueCommand
                    { Kind = SbCommandKind.MoveY, StartMs = t0, EndMs = t1, Start = (float)a.Y, End = (float)b.Y });
                commands.Add(new SbValueCommand
                {
                    Kind = SbCommandKind.VectorScaleX, StartMs = t0, EndMs = t1, Start = (float)a.ScaleX,
                    End = (float)b.ScaleX
                });
                commands.Add(new SbValueCommand
                {
                    Kind = SbCommandKind.VectorScaleY, StartMs = t0, EndMs = t1, Start = (float)a.ScaleY,
                    End = (float)b.ScaleY
                });
                commands.Add(new SbValueCommand
                {
                    Kind = SbCommandKind.Rotate, StartMs = t0, EndMs = t1, Start = (float)a.Rotation,
                    End = (float)b.Rotation
                });
            }

            if (!fadeConstant)
                commands.Add(new SbValueCommand
                    { Kind = SbCommandKind.Fade, StartMs = t0, EndMs = t1, Start = a.Alpha, End = b.Alpha });
            if (!colourConstant)
        ...
        if (!positionConstant)
        {
            if (FlagConstant(SbCommandKind.FlipH, absStart, absEnd))
            {
                if (first.FlipH)
                    commands.Add(new SbFlagCommand { Kind = SbCommandKind.FlipH, StartMs = absStart, EndMs = absEnd });
            }
            ...
            {
                commands.AddRange(FlagWindows(SbCommandKind.FlipH, samples, absEnd, s => s.FlipH));
            }

            if (FlagConstant(SbCommandKind.FlipV, absStart, absEnd))
            {
                if (first.FlipV)
                    commands.Add(new SbFlagCommand { Kind = SbCommandKind.FlipV, StartMs = absStart, EndMs = absEnd });
            }
            ...
            {
                commands.AddRange(FlagWindows(SbCommandKind.FlipV, samples, absEnd, s => s.FlipV));
            }
        }
    ...
    /// <summary>
    ///     P (flag) commands have no persistent "value" — StartMs==EndMs means
    ...
    ///     [windowStart,windowEnd) span instead.
    /// </summary>
    private static IEnumerable<SbCommand> FlagWindows(SbCommandKind kind, List<(double T, TileSample S)> samples,
        double absEnd, Func<TileSample, bool> select)
    {
    ...
    private TileSample SampleAt(double absT, double baseCenterX, double baseCenterY, double baseScale)
    {
        ...
        return new TileSample(x, y, baseScale * sx, baseScale * sy, theta, flipH, flipV, alpha, new SbColor(r, g, b),
            additive);
    }

    /// <summary>
    ///     True iff this one property is unchanged across the whole [start,end] span:
    ...
    ///     command that moves out and back entirely within [start,end].
    /// </summary>
    private bool ScalarConstant(SbCommandKind kind, float defaultValue, double start, double end)
    {
        ...
        if (!CommandEvaluator.EvaluateScalar(_group, kind, end, defaultValue).IsEqual(s))
            return false;
        ...
        foreach (var c in _group)
        {
            ...
            if (v.StartMs > start && v.StartMs < end &&
                !CommandEvaluator.EvaluateScalar(_group, kind, v.StartMs, defaultValue).IsEqual(s))
                return false;
            if (v.EndMs > start && v.EndMs < end &&
                !CommandEvaluator.EvaluateScalar(_group, kind, v.EndMs, defaultValue).IsEqual(s))
                return false;
        }
    ...
    private readonly record struct TileSample(
        double X,
        double Y,
        double ScaleX,
        double ScaleY,
