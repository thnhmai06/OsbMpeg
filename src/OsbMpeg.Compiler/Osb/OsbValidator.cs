...
/// <summary>Round-trip sanity check: decode what we just wrote with OsuParsers' own
/// parser and confirm the object count matches.</summary>
public static class OsbValidator
{
    public static void Validate(string osbPath, SbDocument doc)
    {
        ...
        var decodedCount = decoded.BackgroundLayer.Count + decoded.FailLayer.Count
            + decoded.PassLayer.Count + decoded.ForegroundLayer.Count + decoded.OverlayLayer.Count;
